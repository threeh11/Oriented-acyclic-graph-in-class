...
void Graph :: clearPointers(class Node* deletedRoot)
{
    for (int i = 0; i < 10; i++)
    {
        if (before->pointers[i] == deletedRoot)
        {
            before->pointers[i] = NULL;
        }
    }
}
